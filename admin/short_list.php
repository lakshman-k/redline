...
<table class="table table-bordered">
    ...
    $qry=mysql_query("SELECT * FROM `applied` a, `user` u where a.uid = u.uid");
    while($row=mysql_fetch_array($qry))
    {

        echo'<tr><td>'.$row['a.uid'].'</td><td>'.$row['u.email'].'</td><td>'.$row['a.marks'].'</td><td>'.$row['a.jid'].'</td><td>'.$row['a.resume'].'</td><td>';
        if ($row['status']=='Blocked') {
            echo '<a class="btn btn-danger" href="blockcand.php?id='.$row['uid'].'&& Unblock"> Unblock</a>';
        }
        else{
            echo '<a class="btn btn-warning" href="blockcand.php?id='.$row['uid'].'&&Block"> Block</a>';
        }
